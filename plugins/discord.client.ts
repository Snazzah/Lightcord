...
const discordPlugin: Plugin = (context, inject) => {
  const Discord: DiscordPlugin = {
    init(opts: LoadArgumentsOpts) {
      ...
      if (opts.membersIntent) options.intents.push('guildMembers');
      if (opts.presenceIntent) options.intents.push('guildPresences');
      ...
      this.client.on('ready', () =>
        console.info(...LOG_PREFIX, 'All shards ready.')
      );
      this.client.on('disconnect', () =>
        console.warn(...LOG_PREFIX, 'All shards Disconnected.')
      );
      this.client.on('reconnecting', () =>
        console.warn(...LOG_PREFIX, 'Reconnecting client.')
      );
      ...
      // Shard Events
      this.client.on('connect', (id) =>
        console.info(...LOG_PREFIX, `Shard ${id} connected.`)
      );
      this.client.on('error', (error, id) =>
        console.error(...LOG_PREFIX, `Error in shard ${id}`, error)
      );
      this.client.on('hello', (_, id) =>
        console.info(...LOG_PREFIX, `Shard ${id} recieved hello.`)
      );
      this.client.on('warn', (message, id) =>
        console.warn(...LOG_PREFIX, `Warning in Shard ${id}`, message)
      );
      this.client.on('shardReady', (id) =>
        console.info(...LOG_PREFIX, `Shard ${id} ready.`)
      );
      this.client.on('shardResume', (id) =>
        console.warn(...LOG_PREFIX, `Shard ${id} resumed.`)
      );
      this.client.on('shardDisconnect', (error, id) =>
        console.warn(...LOG_PREFIX, `Shard ${id} disconnected`, error)
      );
    },
    destroy() {
      if (this.client) {
        this.client
          .eventNames()
          .map((eventName) => this.client?.removeAllListeners(eventName));
